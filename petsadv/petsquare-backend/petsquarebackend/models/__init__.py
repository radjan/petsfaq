...
class ModelMixin(object):
    ...
    def __json__(self, request, pass_col=[], visited_tb=[]):
        #log.debug('visit_tb: %s' % visited_tb)
        ...
        for k,value in obj_dict.items():
            ...
            if k in pass_col:
                continue
            if all ([k in foreignkeys.keys(), foreignkeys.get(k, 'no foreignkeys mapping table') not in visited_tb]):
                dfs_visited_tb = list(visited_tb)
                dfs_visited_tb.append(foreignkeys[k])
                value = self.get_by_id(value).__json__(request, visited_tb=dfs_visited_tb)
                k = foreignkeys[k]
